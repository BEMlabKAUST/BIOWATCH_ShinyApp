...
The user has an option to download sequences from the NCBI, BOLD or SILVA databases depending on the gene of interest. For example, for the mitochondrial cytochrome oxidase I gene the user may decide to download sequences from the NCBI and BOLD databases while for the 18S rRNA gene the NCBI and SILVA database would be more appropriate. If the NCBI database is selected then a user defined search criteria are required (e.g. coi[gene] or 18S[All Fields]). Multiple choices can be added (e.g. coi[gene], co1[gene], mtcoi[gene]) to increase the sensitivity of the search or for more complicated search requirements the user can put in a custom query. Note that downloading from NCBI and BOLD are subject to rate limits. While the app has been designed to stay below these limits there are times when downloads can fail due to these limits or due to the maintainence of these databases making them unavailable for download temporarily.
